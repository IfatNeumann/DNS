...
def findAnswerResolver(data,sender_info,s,source_ip,source_port):
    ...
    if findInCache(key) == True and timeStampCheck(key):
        return True

    # while the key is not in the cache or it is but the ttl passed:
    while (findInCache(key) == False or (findInCache(key) and timeStampCheck(key)==False)) and key.find('.') != -1:
        # search NS
        key = key[key.find('.') + 1:]
# ask other servers

    # if found = send message to him
    if findInCache(key) and timeStampCheck(key):
        nsUrl = cache[key][2]
        server = cache[nsUrl]
